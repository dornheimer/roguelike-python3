...
def menu(con, header, options, width, screen_width, screen_height, colors, in_game=True):
    ...
    libtcod.console_set_color_control(libtcod.COLCTRL_1, colors['text_emphasize'], colors['background_default'])
...
def inventory_menu(con, header, player, inventory_width, screen_width, screen_height, colors):
    ...
    if len(player.inventory.items) == 0:
        ...
        for item in player.inventory.items:
            if item in player.equipment.equipped:
                libtcod.console_set_color_control(libtcod.COLCTRL_3, colors['text_desaturate'], colors['background_default'])
                options.append(f'%c{item.name} (equipped)%c' % (libtcod.COLCTRL_3, libtcod.COLCTRL_STOP))
            else:
                libtcod.console_set_color_control(libtcod.COLCTRL_2, colors['text_default'], colors['background_default'])
                options.append(f'%c{item.name}%c' % (libtcod.COLCTRL_2, libtcod.COLCTRL_STOP))

    menu(con, header, options, inventory_width, screen_width, screen_height, colors)


def equipment_menu(con, header, player, equipment_menu_width, screen_width, screen_height, colors):
    ...
    libtcod.console_set_color_control(libtcod.COLCTRL_1, colors['text_emphasize'], colors['background_default'])
    libtcod.console_set_color_control(libtcod.COLCTRL_2, colors['text_desaturate'], colors['background_default'])
    libtcod.console_set_color_control(libtcod.COLCTRL_3, colors['text_info'], colors['background_default'])
    libtcod.console_set_color_control(libtcod.COLCTRL_4, colors['text_info_alt'], colors['background_default'])
...
def main_menu(con, background_image, screen_width, screen_height, colors, in_game=False):
    ...
    menu(con, '', ['new game', 'continue last game', 'quit'], 24, screen_width, screen_height, colors, in_game)


def level_up_menu(con, header, player, menu_width, screen_width, screen_height, colors):
    """Show level up menu with option to choose from."""
    options = ['Constitution (+20 HP, from {0})'.format(player.fighter.max_hp),
                'Strength (+1 attack, from {0})'.format(player.fighter.power),
                'Agility (+1 defense, from {0})'.format(player.fighter.defense)]

    menu(con, header, options, menu_width, screen_width, screen_height, colors)


def character_screen(player, character_screen_width, character_screen_height, screen_width, screen_height, colors):
    """Show character_screen with basic information."""
    window = libtcod.console_new(character_screen_width, character_screen_height)

    libtcod.console_set_default_foreground(window, colors['text_default'])
    libtcod.console_set_color_control(libtcod.COLCTRL_1, colors['text_info_alt'], libtcod.black)
    libtcod.console_set_color_control(libtcod.COLCTRL_2, colors['text_emphasize'], libtcod.black)
...
def message_box(con, header, width, screen_width, screen_height, colors):
    """Show message box."""
    menu(con, header, [], width, screen_width, screen_height, colors)


def description_box(con, description, width, screen_width, screen_height, x, y, colors):
